...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        filtered_df = spacex_df[spacex_df['Launch Site']== entered_site].groupby(['class']).size()/spacex_df[spacex_df['Launch Site']==entered_site]['class'].size
        ...
        # fig = px.pie(filtered_df, values='class', 
        # names='Launch Site', 
        # title='Total Success for one site')
        return fig
# TASK 4:
...
def get_scatter_chart(entered_site, payload_range):
    if entered_site == 'ALL':
        ...
        fig = px.scatter(filtered_df, x="Payload Mass (kg)", y="class", color="Booster Version Category", title=("Correlation between payload and success for launch site " + entered_site))
        return fig
